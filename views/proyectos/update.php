...
<div class="conten-crud">
	...
	<h1 class="title-crud">Modificar Proyectos</h1>
	...
	<?php $form = ActiveForm::begin(['enableClientValidation'=>false]); ?>
		...
		<div class="row">
			...
			<div class="con">
				<?= $form->field($model, 'id_comunidad')->textInput()->hiddenInput()->label(false) ?>
            ...
            <div class="datos-comunidades" style="width: 98%;">
                <?php
                foreach ($comunidades as $comunidades) {
                    ?>
                    <p><strong>Nombre de la comunidad: </strong><?= $comunidades['nombre'] ?></p>
                    <p><strong>Dirección: </strong><?= $comunidades['direccion'] ?></p>
                    <p><strong>Estado: </strong><?= $comunidades['estado'] ?> <strong>Municipio: </strong><?= $comunidades['municipio'] ?> <strong>Parroquia: </strong><?= $comunidades['parroquia'] ?></p>
                    <?php
                }
                ?>
            </p>
		...
		<div class="row">
            ...
            <h3 id="title-estudiante" style="text-align: center;">Estudiantes que ejecutan el proyecto</h3>
            <table style="width: 98%; margin: auto;" class="table table-bordered table-striped table-hover">
                <tr id="encabezado">
                    <th>Cédula</th>
                ...
                <tr>
                    ...
                    <td><?= $estudiantes['primer_nombre'] ?> <?= $estudiantes['segundo_nombre'] ?> <?= $estudiantes['primer_apellido'] ?> <?= $estudiantes['segundo_apellido'] ?></td>
            ...
            if(empty($estudiantes))
            {
                ...
                <h4 style="text-align: center; padding-bottom: 20px;">¡Debes agregar por lo menos un estudiante!</h4>
                <script>
                    document.getElementById("encabezado").style.display = 'none';
                    document.getElementById("title-estudiante").style.display = 'none';
                </script>
                <?php
            }
            ?>
		...
		<button type="button" class="btn btn-success" id="button_registrar_proyecto">Actualizar Proyecto</button>
	<?php ActiveForm::end(); ?>
    ...
    //$("#proyectos-fecha_fin").attr('readonly', "readonly");
     ...
     //--------------------------------------------------------
      ...
      $("#proyectos-id_municipio").change(function(event) {
       ...
       //                  document.getElementById(integrante).innerHTML = '<td class="cedula_integrante" id="'+response.data.cedula+'">'+response.data.cedula+'</td><td>'+response.data.primer_nombre+' '+response.data.segundo_nombre+' '+response.data.primer_apellido+' '+response.data.segundo_apellido+'</td><td>'+response.data.especialidad+'</td><td style="text-align:center;"><button type="button" style="background:#dc3545; color:#fff; border:none;" id="accion_1" class="accion_1" class="btn btn-danger"><i class="fas fa-user-times"></i> Remover Asignacion</button></td>';
       ...
       $("#button_registrar_proyecto").click(function(event) {
            ...
            AlertSigepsiConfirm("¿Estas seguro que deseas modificar este proyecto?", "¡Si confirmas el proyecto sera modificado!", "fas fa-exclamation-circle", "", "Cancelar", "Modificar");
            ...
            // var integrante_1                = document.getElementById("campo_integrante_1").value;
            // var integrante_2                = document.getElementById("campo_integrante_2").value;
            // var integrante_3                = document.getElementById("campo_integrante_3").value;
            // var integrante_4                = document.getElementById("campo_integrante_4").value;
            // var integrante_5                = document.getElementById("integrante_5").value;
            ...
            $("#si").click(function(event) {
                ...
                var url = "sigepsi/web/index.php?r=proyectos/updateproyecto";
                ...
                .done(function(response) {
                    ...
                    if (response.data.message == "El proyecto ha sido modificado.") 
                    {
                        document.querySelector(".preloader").style.display = 'none';
                    AlertSigepsi("Proyecto modificado exitosamente", "<a class='btn btn-primary' href='http://localhost:8080/sigepsi/web/index.php?r=proyectos%2Fupload&id="+id_proyecto+"'>Para modificar su propuesta de proyecto clickea aquí</a>", "fas fa-check", "success");
                ...
                .fail(function() {
                    console.log("error");
                        document.querySelector(".preloader").style.display = 'none';
                        AlertSigepsi("Ocurrió un error al modificar el proyecto", "¡Intenta nuevamente!", "fas fa-times-circle", "warning");
                });   
            });
        });
